...
function changePage(event, searchValue = false) {
  ...
  if (!searchClassDisable(event.target.classList)) {
    ...
    if (event.target.textContent === "next") {
      ...
      let activeChildren = arrChildren.find(
        (el) => el.className == "tui-page-btn tui-is-selected"
      );
      ...
      function go(obj) {
        if (page * countOfElements > 20) {
          ...
          if (this.ElementsOnCurrentPage >= countOfElements) {
            this.firstIndexOfElements =
              this.ElementsOnCurrentPage - countOfElements;
            ...
            this.ElementsOnPageBefore =
              countOfElements - this.ElementsOnCurrentPage;
            this.firstIndexOfElements = 20 - this.ElementsOnPageBefore;
            this.lastIndexOfElements = this.ElementsOnCurrentPage;
          }
        } else {
          this.lastIndexOfElements = page * countOfElements;
          this.lastIndexOfElements - countOfElements < 0
            ? (this.firstIndexOfElements = 0)
            : (this.firstIndexOfElements =
                this.lastIndexOfElements - countOfElements);
        }
        return obj;
      }
    ...
    const QUERY = async function (obj) {
      ...
      if (!obj.isElementsOnPageBefore) {
        await getMoviesData(getFetch(`${obj.currentPageOnServer}`)).then(
          (data) => {
            return (draftArray = [
              ...data.slice(obj.firstIndexOfElements, obj.lastIndexOfElements),
            ]);
          }
        );
      }
      const multipleFetch = async function () {
        const pageBefore = await getMoviesData(
          getFetch(`${obj.currentPageOnServer - 1}`)
        ).then((data) => data.slice(obj.firstIndexOfElements));
        const curentPage = await getMoviesData(
          getFetch(`${obj.currentPageOnServer}`)
        ).then((data) => data.slice(0, obj.lastIndexOfElements));
        return [pageBefore, curentPage];
      };
      //! елементи на 2(двох) сторінка на сервері --->  [TRUE]
      if (obj.isElementsOnPageBefore) {
        await multipleFetch().then((data) => {
