...
//! зробити перевірку якщо ми натиснули на стрілочку


  // let cPage = [...event.currentTarget.children];
  // let bPage = cPage.find(el => el.className == "tui-page-btn tui-is-selected");
  // let fff = bPage.textContent;
  // let bPage = cPage.filter(el => el == "strong.tui-page-btn.tui-is-selected");
  ...
  //todo елементи на 2(двох) сторінка на сервері --->  [FALSE]
  ...
  //todo елементи на 2(двох) сторінка на сервері --->  [TRUE]
      if(obj.isElementsOnPageBefore) {
        await multipleFetch()
          .then(data => {
